...
let INITFACE_SERVER_LOCATION_TEMP = config.get(
  "intifaceServerLocation",
  "localhost:12345"
)
const INITFACE_SERVER_LOCATION = INITFACE_SERVER_LOCATION_TEMP.includes(":")
  ? INITFACE_SERVER_LOCATION_TEMP
  : INITFACE_SERVER_LOCATION_TEMP + ":12345"
...
function updateDeviceVibration(device: ButtplugClientDevice, touched: boolean) {
  ...
  if (status !== undefined) {
    ...
    if (touched) {
      if (
        status.level < VIBRATION_MAX &&
        new Date().valueOf() - status.lastThresholdChange.valueOf() >
          VIBRATION_STEP_LENGTH
      ) {
        status.level = Math.min(
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const updateDeviceListCommand = vscode.commands.registerCommand(
    "prohe.updatedevicelist",
    updateDeviceList
  )
